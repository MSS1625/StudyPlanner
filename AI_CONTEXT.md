...
| فایل | مسئولیت |
|---|---|
| `models.py` | ۷ مدل: `Subject`, `Exam`, `StudyPlan`, `StudyLog`, `UserSecurityProfile` (از 2026-09-10: یک‌به‌یک به User؛ مهرِ زمانِ آخرینِ تغییرِ رمز — محرکِ ابطالِ توکن در `authentication.py`؛ نکته‌ی ۶.۱۷) و `SecurityEvent` (از 2026-09-12: لاگِ خصوصیِ رویدادهایِ امنیتیِ هر کاربر — پنج نوع + IP/UA + هرسِ ۲۰۰تایی؛ `record()` تنها نقطه‌ی نوشتن؛ نکته‌ی ۶.۱۹؛ از 2026-09-14 سه نوعِ 2FA هم دارد و از 2026-09-16 نوعِ `login_new_address`) و `KnownLoginAddress` (از 2026-09-16: دفترِ نشانی‌هایِ شناخته‌شده‌ی هر کاربر — قیدِ یکتاییِ user+ip، هرسِ ۳۰تایی بر اساسِ آخرینِ دیدار، `register()` تنها نقطه‌ی نوشتن با خروجیِ سه‌حالته؛ نکته‌ی ۶.۲۱) |
| `serializers.py` | تبدیل مدل↔JSON + اعتبارسنجی؛ فیلدهای محاسباتی (پیشرفت) اینجا ساخته می‌شوند |
| `views.py` | تمام ViewSetها/Viewهای API؛ شاملِ الگوریتمِ داشبورد و برنامه‌ریزی در سطحِ درخواست؛ از 2026-09-12 تابعِ مشترکِ `_invalidate_all_sessions` (ابطالِ دولایه‌ی نشست — نکته‌ی ۶.۱۸) + دو ویوی بازیابیِ رمز (request/confirm) + ویوی `security_events` و ایمیلِ اطلاع‌رسانیِ `_send_password_notification` (نکته‌ی ۶.۱۹) + از 2026-09-14 دروازه‌ی 2FA در login و چهار ویوی status/setup/confirm/disable و `_verify_totp_code`/`_send_2fa_notification` (نکته‌ی ۶.۲۰) + از 2026-09-16 دروازه‌ی هشدارِ نشانیِ جدید در login و `_new_address_alert_allowed`/`_send_new_address_notification` (نکته‌ی ۶.۲۱) |
...
| `totp.py` | ورودِ دومرحله‌ای (از 2026-09-14): TOTP خالصِ RFC 6238 — `generate_secret`/`totp_code`/`verify_totp`/`build_otpauth_uri` (هیچ ORM/وابستگی‌ای ندارد؛ نکته‌ی ۶.۲۰) |
...
| `tests.py` | ۳۴۲ تستِ خودکارِ Django/DRF: هشت کلاسِ API (Auth تا الگوریتم؛ Auth شامل تست‌های تمدیدِ توکن) + کلاسِ `JWTTokenRotationBlacklistTests` (چرخش/لیستِ سیاه/خروجِ سرور-محور) + کلاسِ `PaginationAPITests` (صفحه‌بندیِ اختیاری) + کلاسِ `SettingsEnvVarsTests` (متغیرهایِ محیطیِ Production؛ شاملِ بوتِ واقعیِ مفسرِ جدا) + کلاسِ `StudyPlanUniqueConstraintTests` با `TransactionTestCase` برای قیدِ DB + کلاس‌های `StudyLogConcurrencyLockTests`/`StudyLogOrphanDeleteTests` (قفلِ هم‌زمانیِ select_for_update و حذفِ یتیم؛ نکته‌ی ۶.۱۲) + کلاس‌های `DatabaseUrlSettingsTests`/`PostgresForUpdateTests` (متغیرِ DATABASE_URL + قفلِ FOR UPDATE در PostgreSQL؛ نکته‌ی ۶.۱۳) + کلاس‌های `I18nAcceptLanguageTests`/`I18nCatalogIntegrityTests` (ترجمه‌ی پیام‌ها با Accept-Language + سلامتِ کاتالوگِ locale/en؛ نکته‌ی ۶.۱۴) + کلاس‌های `MLCalibrationMathTests`/`MLTrainingDataTests`/`MLPredictionsAPITests` (ریاضیاتِ خالص + داده‌ی آموزشی + endpointِ پیش‌بینی؛ نکته‌ی ۶.۱۵) + کلاس‌های `HealthEndpointTests`/`ThrottlingAPITests`/`ThrottleDevSafeDefaultsTests`/`ThrottleAndSecuritySettingsTests`/`SecurityHeadersResponseTests` (سلامت + محدودسازیِ نرخ + هدرهایِ امنیتیِ شرطی؛ نکته‌ی ۶.۱۶) + کلاس‌های `RegisterPasswordPolicyTests`/`ChangePasswordAPITests`/`StaleAccessTokenInvalidationTests`/`UserSecurityProfileModelTests` (سیاستِ رمز + تغییرِ رمز + ابطالِ توکن‌هایِ قبل ازِ تغییر + مدلِ پروفایل؛ نکته‌ی ۶.۱۷) + کلاس‌های `PasswordResetRequestAPITests`/`PasswordResetThrottleTests`/`PasswordResetConfirmAPITests`/`PasswordResetSessionInvalidationTests`/`PasswordResetSettingsTests` (بازیابیِ رمز: ضدِ کشفِ حساب + ماتریسِ رد/پذیرش + یک‌بارمصرف/انقضا + ابطالِ نشست از مسیرِ کاملِ API + بوتِ واقعیِ ایمیل؛ نکته‌ی ۶.۱۸) + کلاس‌های `SecurityEventModelTests`/`LoginAuditRecordingTests`/`SecurityEventsAPITests`/`SecurityNotificationEmailTests`/`SecurityAuditAntiEnumerationTests` (لاگِ رویدادهایِ امنیتی: مدل/ضبط در پنج نقطه/endpoint + جداسازی/ایمیلِ اطلاع‌رسانی/رگرسیونِ ضدِ کشف؛ نکته‌ی ۶.۱۹) + کلاس‌هایِ `TOTPMathTests`/`TwoFactorSetupConfirmAPITests`/`TwoFactorLoginFlowTests`/`TwoFactorThrottleTests`/`TwoFactorDisableAPITests`/`TwoFactorPasswordResetTests`/`TwoFactorNotificationEmailTests` (ورودِ دومرحله‌ای؛ نکته‌ی ۶.۲۰) + کلاس‌های `KnownLoginAddressModelTests`/`NewAddressAlertAPITests` (هشدارِ ورود از نشانیِ جدید؛ نکته‌ی ۶.۲۱)؛ اجرا با `python manage.py test planner` |
| `migrations/0001` تا `0008` | تاریخچه‌ی واقعیِ تکاملِ اسکیمای دیتابیس (تاریخ‌ها در `CHANGELOG.md`) |

۶.۲۰ **ورودِ دومرحله‌ای (TOTP) — قراردادهایِ غیرقابل‌مذاکره (از 2026-09-14).** (۱) **پرچمِ `requires_2fa` فقط بعد ازِ رمزِ درست:** پاسخِ login برایِ رمزِ غلط/کاربرِ ناموجود بایت‌به‌بایت همانِ ۴۰۱ عمومیِ قبلی است — پرچم هیچ کانالِ کشفِ حسابی باز نمی‌کند؛ تست‌های `test_wrong_password_has_no_2fa_flag`/`test_unknown_user_response_unchanged` این قرارداد را قفل کرده‌اند. (۲) **کد یک‌بارمصرف است:** کدِ پذیرفته‌شده در login همان لحظه در `last_used_totp_step` ثبت می‌شود و گام‌هایِ <= آن همیشه رد می‌شوند (RFC 6238 §5.2)؛ استثنایِ آگاهانه: کدِ مصرف‌شده در «تأییدِ فعال‌سازی» نمی‌سوزد تا اولین ورود بی‌درنگ ممکن باشد. (۳) **setup نباید مهرِ `password_changed_at` را بازنویسی کند:** مسیرِ create رکورد با مبنایِ بی‌اثرِ ۲۰۰۰ (`_EPOCH_BASELINE`) و مسیرِ update فقط با `update_fields=['totp_secret','totp_confirmed_at','last_used_totp_step']` — دامِ `update_or_create` با defaults یعنی احیایِ توکن‌هایِ مُرده‌ی بعد ازِ تغییرِ رمز؛ دو رگرسیون‌تست نگهش می‌دارند. (۴) **فعال/خاموش‌کردنِ 2FA نشست‌کُش نیست** (برخلافِ تغییرِ رمز) و (۵) **بازنشانیِ رمز = خاموشیِ 2FA** (ضدِ قفلِ ابدی؛ هزینه‌ی صادقانه: تسخیرِ ایمیل = تسخیرِ حساب — راهِ سخت‌گیرانه‌ترِ «کدهایِ بازیابی» ایده‌ی آینده است). کدها در `planner/totp.py` خالص‌اند (بدونِ ORM) و با شش بردارِ رسمیِ ضمیمه‌ی Bِ RFC 6238 تست شده‌اند؛ پارامترها (SHA1/۶رقم/۳۰ثانیه) عمداً ثابت‌اند چون پیش‌فرضِ همه‌ی اپلیکیشن‌هایِ احرازگرند. رگرسیون‌تست‌ها: هفت کلاسِ `TOTPMathTests`/`TwoFactorSetupConfirmAPITests`/`TwoFactorLoginFlowTests`/`TwoFactorThrottleTests`/`TwoFactorDisableAPITests`/`TwoFactorPasswordResetTests`/`TwoFactorNotificationEmailTests`.
۶.۲۱ **هشدارِ ورود از نشانیِ جدید (2026-09-16) — چهار قراردادِ غیرقابل‌مذاکره.** (۱) **«شناخته‌شده‌بودن» فقط در `KnownLoginAddress` زندگی می‌کند، نه در SecurityEvent** — لاگ هرس می‌شود و اگر مبنا از لاگ خوانده می‌شد، کاربر برایِ دستگاهِ همیشگیِ خودش هشدارِ تکراری می‌گرفت؛ `register()` تنها نقطه‌ی نوشتن است (اتمیک + هرسِ همان‌جا؛ الگوی `SecurityEvent.record`). (۲) **اولینِ ورودِ هر حساب = مبناگذاریِ بی‌هشدار** (`is_first`) — «جدید» (`is_new`) فقط وقتی معنا دارد که چیزی برایِ مقایسه باشد؛ و ثبتِ نشانی فقط بعد ازِ عبورِ کاملِ احراز (رمز + کدِ 2FA) انجام می‌شود — رمزِ غلط نه ردیف می‌سازد نه رویداد. (۳) **پاسخِ login در هیچ سناریویی شکلِ جدیدی پیدا نمی‌کند** (هشدار = ایمیل + رویدادِ `login_new_address`، نه فیلدِ JSON — قراردادِ ضدِ کشفِ حساب؛ تستِ `set(resp.keys()) == {access, refresh}`). (۴) **سقفِ ایمیلِ ۲۴ساعته روی `alert_sent_at`** (`ALERT_DAILY_LIMIT = 5`؛ پنجره‌ی لغزان): رویدادها همیشه ثبت می‌شوند ولی ایمیل زیرِ سقف — «حسابِ درست، صندوقِ سالم» در برابرِ مهاجمِ رمزدار با botnet؛ سهمیه قبل از ارسال خرج می‌شود (شکستِ پایدارِ SMTP نتواند سقف را دور بزند) و شکستِ ارسال هرگز ورود را ۵۰۰ نمی‌کند (الگوی مشترک). IPِ مبنا همان `AuthBurstThrottle.get_ident` است (یک تعریفِ واحد از «از کجا آمد» — پشتِ Nginx همان X-Forwarded-For). رگرسیون‌تست‌ها: دو کلاسِ `KnownLoginAddressModelTests`/`NewAddressAlertAPITests`.
...
۶.۱۹ **لاگِ رویدادهایِ امنیتی (2026-09-12) — چهار قرارداد.** (۱) **تنها یک درِ نوشتن:** هر نوشتن در جدولِ `SecurityEvent` باید از `SecurityEvent.record(user, event_type, request)` بگذرد (IP از `AuthBurstThrottle.get_ident` — همان هویتِ throttle؛ هرسِ اتمیکِ سقفِ `RETENTION_LIMIT=200` رویدادِ هر کاربر در همان ثبت؛ ترتیبِ `-id` برایِ قطعیتِ هم‌ثانیه‌ها). (۲) **لاگ خصوصی است، نه کانالِ عمومی:** ضبطِ رویداد هیچ اثری در پاسخِ HTTPِ مسیرهایِ بی‌لاگین نمی‌گذارد (قراردادِ ضدِ کشفِ حسابِ ۶.۱۸ دست‌نخورده) و خواندنش فقط با توکنِ خودِ کاربر از `GET /api/auth/security/events/` است (فیلتر رویِ `request.user`؛ پارامترِ id وجود ندارد). `login_failed` فقط برایِ کاربرِ موجودِ فعال ثبت می‌شود (`__iexact`؛ ناموجود/غیرفعال = هیچ). (۳) **رویدادِ رمز داخلِ تراکنش، ایمیل بعد از commit:** در `change_password` و `password_reset_confirm` ضبط داخلِ همانِ `transaction.atomic` رمز است (جدایی‌ناپذیر)؛ `_send_password_notification` بعد از commit و فقط برایِ همین دو رویداد فرستاده می‌شود (ورود/تلاشِ ورود عمداً ایمیل نمی‌فرستند — بمبارانِ صندوق با لاگین‌هایِ ناموفق)؛ شکستِ SMTP ثبت و بلعیده می‌شود (`logger.exception`؛ مسیرِ اصلی ۲۰۰ می‌ماند). (۴) **برچسب = ترجمه‌ی بک‌اند، کلید = ثابت:** API هم `type` ماشین‌خوان (کلاینت رندر/فیلتر را رویِ آن می‌سازد) و هم `label` ترجمه‌شده با Accept-Language برمی‌گرداند (`get_event_type_display` رویِ TextChoicesِ gettext_lazy)؛ `?limit=` ۱..۲۰۰ با پیش‌فرضِ ۵۰ و مقدارِ بی‌معنا → پیش‌فرض (نه خطا). رگرسیون‌تست‌ها: کلاس‌های `SecurityEventModelTests`/`LoginAuditRecordingTests`/`SecurityEventsAPITests`/`SecurityNotificationEmailTests`/`SecurityAuditAntiEnumerationTests`.
