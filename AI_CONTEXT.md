...
> - `docs/01_config_and_models.md` تا `docs/04_frontend_logic_and_design.md` — توضیح خط‌به‌خط و فایل‌به‌فایلِ کل پروژه
...
جزئیاتِ فیلد‌به‌فیلد در `README.md` بخش ۶ و `docs/01_config_and_models.md` بخش ۱.۶.

## ۶. نکاتِ حیاتی هنگام ویرایشِ کد (این‌ها را نقض نکن)

۶.۱ **منطقِ کسرِ ساعت در مدل است، نه در View.** `StudyLog.save()` به‌صورتِ خودکار `exam.study_hours_remaining` را کم می‌کند (فقط برای رکوردهای *جدید*، نه ویرایش). اگر جایی منطقِ مشابه در `views.py` اضافه شود، ساعت **دو بار** کم خواهد شد. برعکس: در حالِ حاضر `StudyLog` متدِ `delete()` را Override **نکرده**، پس حذفِ یک لاگ، ساعت را برنمی‌گرداند (این محدودیت عمدی و مستند‌شده است، نه باگِ فراموش‌شده — به `TODO.md` نگاه کن).
...
۶.۶ **`Subject` فیلدهای `total_hours` و `notes` ندارد**، ولی فرمِ `subjects.html` این دو مقدار را می‌فرستد (`app.js`, تابعِ `handleSubjectForm`). سریالایزر همین الان این دو کلید را نادیده می‌گیرد (خطا نمی‌دهد، فقط ذخیره نمی‌کند). این یک ناهماهنگیِ شناخته‌شده‌ی فعلی است (به `TODO.md` نگاه کن)، نه چیزی که باید بدونِ تصمیمِ آگاهانه "دیده نشود".
...
| `docs/01_config_and_models.md` | توضیحِ خط‌به‌خطِ پیکربندی و مدل‌ها |
| `docs/02_server_logic_and_api.md` | توضیحِ خط‌به‌خطِ سریالایزر/ویو/الگوریتم |
| `docs/03_frontend_pages.md` | توضیحِ هر ۷ صفحه‌ی HTML |
| `docs/04_frontend_logic_and_design.md` | توضیحِ `app.js` و `styles.css` |
