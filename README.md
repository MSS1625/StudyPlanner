...
| فناوری | نقش در پروژه |
|---|---|
| **HTML5** | ساختار معناییِ (Semantic) صفحات؛ ۸ صفحه‌ی مجزا (ورود، ثبت‌نام، بازیابیِ رمز، داشبورد، درس‌ها، امتحان‌ها، برنامه‌ی مطالعه، ثبت مطالعه) |
...
| `POST` | `/api/auth/login/` | ورود و دریافت توکن JWT — برایِ کاربرانِ دارایِ ورودِ دومرحله‌ای، فیلدِ اختیاریِ `totp` (کدِ ۶رقمیِ اپلیکیشنِ احرازگر) هم لازم است؛ بدونِ آن، پاسخ ۴۰۱ با پرچمِ `requires_2fa` است (از 2026-09-14) |
...
| `POST` | `/api/auth/password/` | تغییرِ رمزِ عبور با ابطالِ همه‌ی نشست‌های دیگر + صدورِ جفتِ توکنِ تازه (از 2026-09-10) |
| `POST` | `/api/auth/password/reset/` | درخواستِ لینکِ بازیابیِ رمز (identifier = نامِ کاربری یا ایمیل؛ پاسخِ همیشگیِ عمومی — ضدِ کشفِ حساب) (از 2026-09-12) |
| `POST` | `/api/auth/password/reset/confirm/` | تعیینِ رمزِ جدید با uid/token لینکِ ایمیل‌شده — یک‌بارمصرف و زمان‌دار؛ بدونِ صدورِ توکن (ورودِ تازه لازم است) (از 2026-09-12) |
| `GET` | `/api/auth/security/events/` | تاریخچه‌ی رویدادهایِ امنیتیِ حسابِ کاربرِ لاگین‌شده — ورودِ موفق/ناموفق، تغییر/بازنشانیِ رمز، فعال/خاموش‌شدنِ ورودِ دومرحله‌ای و ورود از نشانیِ جدید با زمان، IP و مرورگر (`?limit=` اختیاری؛ برچسب‌ها fa/en با Accept-Language) (از 2026-09-12؛ نوعِ نشانیِ جدید از 2026-09-16) |
| `GET` | `/api/auth/2fa/` | وضعیتِ ورودِ دومرحله‌ایِ کاربرِ لاگین‌شده — `{"enabled": true/false}` (از 2026-09-14) |
| `POST` | `/api/auth/2fa/setup/` | ساختِ کلیدِ تازه‌ی TOTP (base32) + لینکِ otpauth برایِ اپلیکیشنِ احرازگر — تا تأیید با کد، در ورود بی‌اثر (از 2026-09-14) |
| `POST` | `/api/auth/2fa/confirm/` | فعال‌سازیِ ورودِ دومرحله‌ای با اولین کدِ درست + رویدادِ امنیتی + ایمیلِ اطلاع‌رسانی (از 2026-09-14) |
| `POST` | `/api/auth/2fa/disable/` | خاموش‌کردنِ ورودِ دومرحله‌ای با «رمزِ عبور + کدِ فعلی» (دفاعِ دولایه) + رویداد + ایمیل (از 2026-09-14) |
...
| `GET` | `/api/predictions/` | پیش‌بینیِ «ساعتِ واقعیِ موردنیاز» و ریسکِ عقب‌افتادن برایِ امتحان‌هایِ آینده — مؤلفه‌ی یادگیریِ آماریِ کاربر-محور (از 2026-09-09) |
| `GET` | `/api/health/` | پایشِ سلامت برایِ مانیتورینگ/Load Balancer — عمومی، بدونِ throttle؛ ۲۰۰ = فرایند + دیتابیس سالم، ۵۰۳ = دیتابیس ناسالم (از 2026-09-10) |
...
9. **مجموعه‌تستِ خودکار**: ۳۴۲ تستِ Django/DRF در `backend/planner/tests.py` (احراز هویت (شامل تمدیدِ توکن و چرخش/لیستِ سیاه)، CRUD درس/امتحان/گزارش، قفلِ هم‌زمانی و خواندنِ تازه در ثبت/حذفِ گزارش، سناریوهای امنیتیِ جداسازی کاربران، صفحه‌بندیِ اختیاری، تنظیماتِ محیطیِ Production و `DATABASE_URL`، برنامه‌ریز، قیدِ یکتاییِ `StudyPlan` در DB، داشبورد، تستِ واحدِ الگوریتم، قفلِ FOR UPDATE در PostgreSQL، چندزبانیِ Accept-Language/کاتالوگ، ریاضیات/داده/جریانِ کاملِ مؤلفه‌ی یادگیریِ آماری و endpoint سلامت/محدودسازیِ نرخ/هدرهایِ امنیتیِ شرطی، و سیاستِ رمز/تغییرِ رمز/ابطالِ نشست‌ها، و بازیابیِ رمز/ضدِ کشفِ حساب/تنظیماتِ ایمیل، و لاگِ رویدادهایِ امنیتی/ایمیلِ اطلاع‌رسانی، و ورودِ دومرحله‌ای ازِ OTP ریاضی تا جریانِ کامل، و هشدارِ ورود از نشانیِ جدید ازِ مدلِ نشانی‌ها تا سقفِ ایمیل) — اجرای `python manage.py test planner`.
...
18. **پیش‌بینیِ هوشمندِ زمانِ واقعی (مؤلفه‌ی یادگیریِ آماری)**: `GET /api/predictions/` + پنلِ «پیش‌بینیِ هوشمند» در صفحه‌ی برنامه — سیستم از امتحان‌هایِ *گذشته‌ی* خودِ کاربر می‌آموزد که تخمین‌هایِ ساعتی‌اش چند برابرِ واقعیت بوده (مدلِ کالیبراسیونِ β با رگرسیونِ کمینه‌ی مربعات + انقباضِ بیزی؛ پایتونِ خالص، بدونِ وابستگیِ جدید) و برایِ هر امتحانِ آینده «ساعتِ واقعیِ موردنیاز»، نیازِ روزانه و ریسکِ عقب‌افتادن نسبت به ساعتِ آزادِ روزانه‌ی همان کاربر را پیش‌بینی می‌کند؛ کاربرِ بی‌تاریخچه = همان تخمینِ دستیِ خودش (β=۱) (از 2026-09-09).
19. **آمادگیِ استقرارِ واقعی (محدودسازیِ نرخ + سلامت + TLS شرطی)**: دفاعِ brute-force رویِ register/login با سه scope نرخ (`anon`/`user`/`auth` از متغیرهایِ محیطی — پیش‌فرضِ توسعه مؤثراً نامحدود)، endpoint عمومیِ `GET /api/health/` معاف از throttle برایِ مانیتورینگ و هدرهایِ امنیتیِ شرطی (`SECURE_SSL_REDIRECT`/کوکی‌هایِ Secure/HSTS/هدرِ پروکسی) که فقط پشتِ TLS واقعی معنا دارند؛ راهنمایِ گام‌به‌گامِ استقرار (Nginx + Certbot + گنیکورن + systemd) در `05_deployment.md` (از 2026-09-10).
20. **امنیتِ حسابِ کاربری (سیاستِ رمز + تغییرِ رمز با ابطالِ کاملِ نشست‌ها)**: رمزِ ثبت‌نام و تغییرِ رمز از `AUTH_PASSWORD_VALIDATORS` جنگو می‌گذرند (حداقلِ ۸ نویسه، غیرِ رایج، غیرِ عددیِ خالص، غیرِشبیهِ username — پیام‌ها fa/en خودکار) و `POST /api/auth/password/` با گرفتنِ رمزِ فعلی، همه‌ی نشست‌هایِ دیگر را باطل می‌کند: همه‌ی توکن‌هایِ Refreshِ برجسته **و** توکن‌هایِ دسترسیِ صادرشده قبل ازِ تغییر (کلاسِ سفارشیِ `planner/authentication.py` با مقایسه‌یِ `iat`); جفتِ توکنِ تازه در همان پاسخ، نشستِ همین دستگاه را بی‌وقفه نگه می‌دارد؛ دکمه‌ی «تغییر رمز عبور» و مودالِ آن با app.js در همه‌ی صفحات تزریق می‌شود (از 2026-09-10).
21. **بازیابیِ رمزِ فراموش‌شده (لینکِ ایمیلِ یک‌بارمصرف)**: لینکِ «رمز عبور را فراموش کرده‌اید؟» در صفحه‌ی ورود → درخواست با نامِ کاربری یا ایمیل → لینکِ بازیابی (توکنِ امضاشده‌ی جنگو — بدونِ مایگریشن؛ یک‌بارمصرف و یک‌ساعته) در ایمیل → صفحه‌ی `reset-password.html` برایِ تعیینِ رمزِ جدید؛ پاسخِ درخواست عمداً «عمومی» است (ضدِ کشفِ حساب) و بازیابیِ موفق همه‌ی نشست‌هایِ قبلی را باطل می‌کند و توکنی صادر نمی‌کند (ورودِ تازه لازم است). در توسعه، ایمیل‌ها به‌جایِ ارسالِ واقعی در کنسولِ runserver نوشته می‌شوند (پیش‌فرضِ dev-safe)؛ در Production با متغیرهایِ SMTP فعال می‌شوند — جدولِ کامل در بخشِ ۱۰ و راهنمایِ `05_deployment.md` (از 2026-09-12).
22. **لاگِ رویدادهایِ امنیتی (تاریخچه‌ی حساب + ایمیلِ اطلاع‌رسانی)**: هر ورودِ موفق/ناموفق، تغییرِ رمز و درخواست/بازنشانیِ بازیابی با زمان، IP و مرورگر ثبت می‌شوند و از `GET /api/auth/security/events/` فقط برایِ خودِ کاربر خوانده می‌شوند (سقفِ نگه‌داری: ۲۰۰ رویدادِ هر کاربر با هرسِ خودکار در هر ثبت؛ جداسازیِ کامل بینِ کاربرها)؛ دکمه‌ی «فعالیت‌های امنیتی» در همه‌ی صفحاتِ لاگین‌شده مودالِ تاریخچه را باز می‌کند (تاریخِ جلالی/میلادی با زبان). تغییر یا بازنشانیِ رمز به‌علاوه یک ایمیلِ اطلاع‌رسانیِ امنیتی («اگر شما نبودید...» با IPِ فرستنده) به ایمیلِ ثبت‌شده می‌فرستد — در توسعه در کنسولِ runserver و در Production با SMTP؛ شکستِ ارسال مسیرِ اصلی را خراب نمی‌کند (از 2026-09-12).
23. **ورودِ دومرحله‌ای (TOTP — کدِ یک‌بارمصرفِ اپلیکیشنِ احرازگر)**: لایه‌ی دومِ ورود با پیاده‌سازیِ خالصِ RFC 6238 (بدونِ وابستگیِ جدید — سازگار با Google Authenticator و مشابه‌ها): دکمه‌ی «ورودِ دومرحله‌ای» مودالِ فعال‌سازی را باز می‌کند (QR + کلیدِ دستی + تأیید با اولین کدِ درست) و از آن پس ورود علاوه بر رمز، کدِ لحظه‌ایِ ۶رقمی می‌خواهد (گامِ دوم در همان فرمِ ورود ظاهر می‌شود)؛ کد یک‌بارمصرف است (پخشِ مجدد رد می‌شود)، خطایِ ساعت تا ~۳۰ ثانیه با پنجره‌ی ±۱ گام بخشیده می‌شود و خاموش‌کردن دفاعِ دولایه می‌خواهد (رمز + کد)؛ بازنشانیِ رمز با لینکِ ایمیل، 2FA را هم خاموش می‌کند تا گم‌کردنِ هم‌زمانِ گوشی و رمز، حساب را برای همیشه قفل نکند (رویدادها و ایمیلِ اطلاع‌رسانیِ جداگانه دارد) (از 2026-09-14).
24. **هشدارِ ورود از نشانیِ جدید**: اولینِ ورودِ هر حساب «مبنا» را می‌گذارد؛ ورودِ بعدی از نشانی‌ای که حسابِ شما ندیده بود → ایمیلِ هشدار با نشانی/دستگاه و راهِ عملِ دوشاخه‌ای (خودت بودی: هیچ؛ نبودی: بازنشانیِ رمز + روشن‌کردنِ 2FA) + ردیفِ «ورود از نشانیِ جدید» در «فعالیت‌هایِ امنیتی»؛ دستگاهِ همیشگی هیچ‌وقت هشدارِ تکراری نمی‌گیرد (دفترِ نشانی‌ها جدا از لاگِ هرس‌شونده)، ثبتِ نشانی فقط بعد ازِ احرازِ کامل (رمز + کدِ 2FA) و سقفِ ۵ ایمیل در ۲۴ ساعت («حسابِ درست، صندوقِ سالم» — رویدادها همیشه ثبت می‌شوند) (از 2026-09-16).
...
# ۷. (پیشنهادشده) اجرای مجموعه‌تست‌های خودکار — ۳۴۲ تست، بدونِ دست‌زدن به دیتابیسِ اصلی
python manage.py test planner
...
| `DJANGO_ANON_THROTTLE_RATE` | `10000/min` | `120/min` — نرخِ درخواست‌هایِ بی‌احرازِ هویت، به‌ازایِ IP (از 2026-09-10) |
...
| `DJANGO_SECURE_SSL_REDIRECT` | `false` | `1` — ریدایرکتِ http→https؛ **فقط بعد از فعال‌شدنِ TLS** (از 2026-09-10) |
| `DJANGO_COOKIES_SECURE` | `false` | `1` — فلگِ Secure رویِ کوکی‌هایِ Session/CSRF (از 2026-09-10) |
| `DJANGO_HSTS_SECONDS` | `0` (خاموش) | `31536000` (یک سال) — «فقط https»؛ **فقط بعد از TLS مطمئن** (از 2026-09-10) |
| `DJANGO_PROXY_SSL_HEADER` | خالی | `HTTP_X_FORWARDED_PROTO,https` — جنگو از این هدر می‌فهمد ترافیک https بوده (از 2026-09-10) |
| `DJANGO_EMAIL_BACKEND` | `django.core.mail.backends.console.EmailBackend` | `django.core.mail.backends.smtp.EmailBackend` — موتورِ ارسالِ ایمیلِ بازیابی؛ پیش‌فرضِ console یعنی ایمیل در stdoutِ runserver (بی‌SMTP — لینک را از کنسول کپی کنید) (از 2026-09-12) |
| `DJANGO_EMAIL_HOST` | خالی | `smtp.example.com` — سرورِ SMTP (فقط با backendِ SMTP) (از 2026-09-12) |
| `DJANGO_EMAIL_PORT` | `587` | `465` (مثلاً برایِ TLSِ ضمنِ اتصال) (از 2026-09-12) |
| `DJANGO_EMAIL_HOST_USER` | خالی | نامِ کاربریِ SMTP (از 2026-09-12) |
| `DJANGO_EMAIL_HOST_PASSWORD` | خالی | رمزِ SMTP (از 2026-09-12) |
| `DJANGO_EMAIL_USE_TLS` | `true` | `false` — فقط با پورتِ 465/SSLِ ضمنِ اتصال خاموش کنید (از 2026-09-12) |
| `DJANGO_DEFAULT_FROM_EMAIL` | `webmaster@localhost` | `no-reply@example.com` — فرستنده‌ی ایمیلهایِ سیستمی (از 2026-09-12) |
| `DJANGO_PASSWORD_RESET_TIMEOUT` | `3600` (یک ساعت) | عمرِ لینکِ بازیابی به ثانیه — مقدارِ ۰/منفی/غیرعددی بوت را متوقف می‌کند (از 2026-09-12) |
| `DJANGO_FRONTEND_BASE_URL` | `http://127.0.0.1:8000` | `https://planner.example.com` — مبدأِ لینکِ داخلِ ایمیلِ بازیابی (در Production آدرسِ عمومیِ سایت) (از 2026-09-12) |

راهنمایِ گام‌به‌گامِ استقرارِ واقعیِ این متغیرها رویِ سرورِ لینوکس (PostgreSQL + گنیکورن + systemd + Nginx + Certbot + فهرستِ بررسیِ نهایی): مستندِ **`05_deployment.md`** (از 2026-09-10). قالبِ نرخ‌ها `<عدد>/<sec\|min\|hour\|day>` است و مقدارِ خراب بوت را با پیامِ راهنما متوقف می‌کند؛ با `DEBUG=false` و نرخ‌هایِ پیش‌فرض، هشداریِ یک‌خطی رویِ stderr نوشته می‌شود (بوت ادامه می‌یابد).
...
python manage.py test planner     # ۳۴۲ تست — این‌بار روی PostgreSQL
```
...
- **شمارنده‌هایِ throttle در حافظه‌ی هر فرایند (LocMemCache)**: محدودسازیِ نرخِ 2026-09-10 شمارنده‌ها را در کشِ پیش‌فرضِ جنگو (هر فرایند جدا) نگه می‌دارد — پشتِ چند workerِ گنیکورن یعنی «سقفِ هر worker» (با ۲ worker = ~۲× نرخ). برایِ این پروژه (تک‌سرور، کاربرانِ محدود) قابل‌قبول و در `05_deployment.md` بخشِ ۵.۹ صادقانه حساب شده؛ مهاجرت بهِ shared-cache (مثلِ django-redis) روزی لازم شد، طبقِ قانونِ مستندسازی اول در `TODO.md` ثبت شود.

- **یادگیریِ آماریِ سبک (از 2026-09-09)**: مؤلفه‌ی `planner/ml.py` دقیقاً همان ایده‌ی رگرسیونیِ وعده‌داده‌شده را با پایتونِ خالص (بدونِ Scikit-learn و وابستگیِ جدید) پیاده می‌کند: مدلِ کالیبراسیونِ β هر کاربر از امتحان‌هایِ گذشته‌اش می‌آموزد و «ساعتِ واقعیِ موردنیاز» امتحان‌هایِ آینده را پیش‌بینی می‌کند (`GET /api/predictions/`). محدودیتِ صادقانه‌ی باقی‌مانده: بازسازیِ تخمینِ اولیه فرض می‌کند ساعتِ باقی‌مانده فقط از مسیرِ گزارش‌های مطالعه کم شده (ویرایشِ دستیِ امتحان نویز وارد می‌کند) و با داده‌ی کم، انقباضِ بیزی مدل را به‌سمتِ «قبولِ تخمینِ دستی» (β=۱) برمی‌گرداند — هر دو عمدی و مستند در `planner/ml.py`.
...
- ~~عدم پشتیبانی از چند‌زبانی~~: از 2026-09-09 فارسی/انگلیسی در هر دو لایه پشتیبانی می‌شود (بخشِ ۸، قابلیتِ ۱۷)؛ این نکته که از Taskِ چندزبانی جامانده بود در 2026-09-09 اصلاح شد.
