...
| `GET` | `/api/auth/security/events/` | تاریخچه‌ی رویدادهایِ امنیتیِ حسابِ کاربرِ لاگین‌شده — ورودِ موفق/ناموفق، تغییر/بازنشانیِ رمز، فعال/خاموش‌شدنِ ورودِ دومرحله‌ای با زمان، IP و مرورگر (`?limit=` اختیاری؛ برچسب‌ها fa/en با Accept-Language) (از 2026-09-12) |
...
9. **مجموعه‌تستِ خودکار**: ۳۲۴ تستِ Django/DRF در `backend/planner/tests.py` (احراز هویت (شامل تمدیدِ توکن و چرخش/لیستِ سیاه)، CRUD درس/امتحان/گزارش، قفلِ هم‌زمانی و خواندنِ تازه در ثبت/حذفِ گزارش، سناریوهای امنیتیِ جداسازی کاربران، صفحه‌بندیِ اختیاری، تنظیماتِ محیطیِ Production و `DATABASE_URL`، برنامه‌ریز، قیدِ یکتاییِ `StudyPlan` در DB، داشبورد، تستِ واحدِ الگوریتم، قفلِ FOR UPDATE در PostgreSQL، چندزبانیِ Accept-Language/کاتالوگ، ریاضیات/داده/جریانِ کاملِ مؤلفه‌ی یادگیریِ آماری و endpoint سلامت/محدودسازیِ نرخ/هدرهایِ امنیتیِ شرطی، و سیاستِ رمز/تغییرِ رمز/ابطالِ نشست‌ها، و بازیابیِ رمز/ضدِ کشفِ حساب/تنظیماتِ ایمیل، و لاگِ رویدادهایِ امنیتی/ایمیلِ اطلاع‌رسانی) — اجرای `python manage.py test planner`.
...
# ۷. (پیشنهادشده) اجرای مجموعه‌تست‌های خودکار — ۳۲۴ تست، بدونِ دست‌زدن به دیتابیسِ اصلی
...
python manage.py test planner     # ۳۲۴ تست — این‌بار روی PostgreSQL
```
