...
9. **مجموعه‌تستِ خودکار**: ۸۹ تستِ Django/DRF در `backend/planner/tests.py` (احراز هویت (شامل تمدیدِ توکن)، CRUD درس/امتحان/گزارش، سناریوهای امنیتیِ جداسازی کاربران، صفحه‌بندیِ اختیاری، تنظیماتِ محیطیِ Production، برنامه‌ریز، قیدِ یکتاییِ `StudyPlan` در DB، داشبورد و تستِ واحدِ الگوریتم) — اجرای `python manage.py test planner`.
...
# ۷. (پیشنهادشده) اجرای مجموعه‌تست‌های خودکار — ۸۹ تست، بدونِ دست‌زدن به دیتابیسِ اصلی
