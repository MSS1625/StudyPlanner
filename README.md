...
9. **مجموعه‌تستِ خودکار**: ۱۱۶ تستِ Django/DRF در `backend/planner/tests.py` (احراز هویت (شامل تمدیدِ توکن و چرخش/لیستِ سیاه)، CRUD درس/امتحان/گزارش، قفلِ هم‌زمانی و خواندنِ تازه در ثبت/حذفِ گزارش، سناریوهای امنیتیِ جداسازی کاربران، صفحه‌بندیِ اختیاری، تنظیماتِ محیطیِ Production و `DATABASE_URL`، برنامه‌ریز، قیدِ یکتاییِ `StudyPlan` در DB، داشبورد، تستِ واحدِ الگوریتم و قفلِ FOR UPDATE در PostgreSQL) — اجرای `python manage.py test planner`.
...
# ۷. (پیشنهادشده) اجرای مجموعه‌تست‌های خودکار — ۱۱۶ تست، بدونِ دست‌زدن به دیتابیسِ اصلی
...
python manage.py test planner     # ۱۱۶ تست — این‌بار روی PostgreSQL
```
