...
- تفکیک کامل _Backend_ (سرویس داده) از _Frontend_ (رابط کاربری)، به‌گونه‌ای که این دو تنها از طریق HTTP/JSON با هم ارتباط دارند (معماری Decoupled/Headless)
...
- **تقسیم‌زمانِ پیشنهادی (نمودار داشبورد)**: چون مجموع ساعتِ _هر روز_ تقریباً ثابت است، به‌جای آن، سهم _هر درس_ از کل ساعات هفته‌ی پیش‌رو محاسبه و به‌صورت نموداری نرمال‌شده (نسبت به پرکارترین درس) نمایش داده می‌شود.
...
- `Exam.study_hours_remaining` تنها منبعِ «حجم باقی‌مانده» است و به‌صورت خودکار در متد سفارشیِ `StudyLog.save()` کاهش می‌یابد (و هرگز منفی نمی‌شود).
...
| `DELETE`               | `/api/study-logs/{id}/`                | حذف یک گزارش مطالعه                                                        |
...
4. **ثبت مطالعه (Study Log)**: ثبت هر جلسه‌ی مطالعه (امتحان مربوطه، تاریخ، مدت، یادداشت اختیاری) که به‌صورت خودکار روی ساعت باقی‌مانده‌ی همان امتحان و در نتیجه روی درصد پیشرفتِ درس تأثیر می‌گذارد.
...
- **عدم بازگشتِ خودکارِ ساعت هنگام حذف گزارش مطالعه**: در حال حاضر حذف یک `StudyLog` باعث بازگشتِ ساعتِ کسرشده به `study_hours_remaining` نمی‌شود (برای جلوگیری از پیچیدگی و خطاهای احتمالیِ محاسباتی)؛ این مورد در رابط کاربری به کاربر اطلاع داده می‌شود.
...
- **نبودِ آزمون‌های خودکار (Automated Tests)**: فایل `tests.py` هنوز خالی است؛ افزودن تست‌های واحد برای الگوریتم `generate_study_plan` و APIها گامِ بعدیِ طبیعی برای افزایش اطمینان از صحت سیستم است.
...
تهیه کننده: محمد صادق صداقت مهندسی حرفه ای کامپیوتر دانشگاه ملی مهارت استان فارس شهید باهنر
