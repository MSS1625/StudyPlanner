...
9. **مجموعه‌تستِ خودکار**: ۷۹ تستِ Django/DRF در `backend/planner/tests.py` (احراز هویت، CRUD درس/امتحان/گزارش، سناریوهای امنیتیِ جداسازی کاربران، صفحه‌بندیِ اختیاری، برنامه‌ریز، قیدِ یکتاییِ `StudyPlan` در DB، داشبورد و تستِ واحدِ الگوریتم) — اجرای `python manage.py test planner`.
...
# ۷. (پیشنهادشده) اجرای مجموعه‌تست‌های خودکار — ۷۹ تست، بدونِ دست‌زدن به دیتابیسِ اصلی
