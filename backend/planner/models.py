...
class StudyLog(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            # «ساعتِ واقعاً کسرشده» را «قبل» از ذخیره‌ی خودِ گزارش حساب می‌کنیم
            # تا همراهِ همان INSERT در دیتابیس بنشیند (مبنای بازگرداندنِ ساعت
            # در delete() پایین). عددِ کسرشده = کمینه‌ی «ساعتِ مطالعه» و
            # «ساعتِ موجودِ امتحان»؛ یعنی اگر فقط ۲ ساعت مانده باشد و کاربر
            # ۵ ساعت ثبت کند، دقیقاً ۲ ساعت کسر می‌شود (نه بیشتر).
        ...
        # کلِ عملیاتِ «ذخیره‌ی گزارش + کسرِ ساعتِ امتحان» یکپارچه و اتمیک است:
        # اگر قدمِ دوم شکست بخورد، قدمِ اول هم Rollback می‌شود و گزارشی
        # نیمه‌کاره (بدونِ کسرِ ساعت) باقی نمی‌ماند.
        with transaction.atomic():
            super().save(*args, **kwargs)

            # اگر رکورد جدید بود، ساعتِ کسرشده را از امتحان مربوطه کم می‌کنیم
            # (برای رکوردهای ویرایش‌شده این کار تکرار نمی‌شود، تا کسرِ دوباره
    ...
    def delete(self, *args, **kwargs):
        ...
        (بستنِ آیتمِ Medium Priorityِ TODO.md، 2026-09-03.)
        ...
        """
        # قبل از حذف، مرجعِ امتحان و مبلغِ بازگشتی را در متغیر می‌گیریم؛
        # بعد از super().delete() دیگر نمی‌خواهیم به رابطه‌های رکوردِ در
        # حالِ حذف تکیه کنیم.
        # (getattr با مقدارِ پیش‌فرض: اگر رکوردِ یتیمی با کلیدِ خارجیِ شکسته
        # به این متد رسید، exam برابرِ None می‌شود و فقط خودِ رکورد حذف
        # می‌شود، بدونِ خطا.)
